      ...
      >
        {points.map((point) => (
          <motion.div
            key={point.title}
            variants={fadeInUp}
            className={`rounded-2xl p-6 ${dark ? 'glass-card hover:border-accent/20' : 'bg-gray-50 border border-gray-100 hover:border-gray-200 hover:shadow-lg'} transition-all duration-300`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-4 ${dark ? 'bg-accent/10' : 'bg-accent/10'}`}>
